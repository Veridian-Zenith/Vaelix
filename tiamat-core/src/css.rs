...
#[derive(Clone, Debug)]
pub struct NodeHandle(pub Rc<markup5ever_rcdom::Node>);
...
impl Hash for NodeHandle {
    fn hash<H: Hasher>(&self, state: &mut H) {
        (&*self.0 as *const markup5ever_rcdom::Node).hash(state)
    }
}
...
impl CSSParser {
    ...
    pub fn apply_styles(
        ...
        rules: &[Rule],
        dom: &Handle,
    ) -> HashMap<NodeHandle, HashMap<String, String>> {
            ...
            for selector in &rule.selectors {
                for element in &elements {
                    if self.matches_selector(element, selector) {
                        let handle = NodeHandle(element.clone());
                        let element_styles = styles.entry(handle).or_insert_with(HashMap::new);
            ...
            if let NodeData::Element { attrs, .. } = &element.data {
                ...
                for attr in attrs.iter() {
                    if attr.name.local.to_string() == "class" {
                        ...
                        let classes: Vec<&str> = class_value.split(' ').collect();
...
mod tests {
    ...
    fn test_style_application() {
        ...
        // Check logo style
        if let Some(logo_styles) =
            styles.get(&dom.children.borrow()[0].children.borrow()[0].children.borrow()[0])
        {
