...
class SiebenApp : public CefApp, public CefClient, public CefBrowserProcessHandler {
public:
    SiebenApp() = default;
    ...
    void OnContextInitialized() override {
        ...
        command_line->AppendSwitch("enable-features=VaapiVideoDecoder,VaapiVideoEncoder");
        command_line->AppendSwitch("disable-features=UseChromeOSDirectVideoDecoder");
        ...
        // Create browser
        CefBrowserHost::CreateBrowser(window_info, this, "https://www.google.com", browser_settings, nullptr, nullptr);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (!wayland_handler.SetupVAAPI()) {
        std::cerr << "Warning: Failed to setup VA-API, continuing without hardware acceleration" << std::endl;
    }

    if (!wayland_handler.SetupVulkan()) {
        std::cerr << "Warning: Failed to setup Vulkan, continuing with fallback rendering" << std::endl;
    }
