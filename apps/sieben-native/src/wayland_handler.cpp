...
WaylandHandler::~WaylandHandler() { Cleanup(); }
...
bool WaylandHandler::SetupWaylandConnection() {
  ...
  static const wl_registry_listener registry_listener = {
      RegistryGlobalHandler, RegistryGlobalRemoveHandler};
...
bool WaylandHandler::SetupEGL() {
  ...
  EGLint num_configs;
  if (!eglChooseConfig(egl_display_, config_attrs, &egl_config_, 1,
                       &num_configs) ||
      num_configs == 0) {
...
bool WaylandHandler::CreateSurface(int width, int height) {
  ...
  egl_surface_ = eglCreateWindowSurface(
      egl_display_, egl_config_, (EGLNativeWindowType)egl_window_, nullptr);
...
bool WaylandHandler::SetupVAAPI() {
  ...
  VAStatus va_status =
      vaInitialize(va_display_, &major_version, &minor_version);
  if (va_status != VA_STATUS_SUCCESS) {
    std::cerr << "Failed to initialize VA: " << vaErrorStr(va_status)
              << std::endl;
    close(drm_fd);
    return false;
  }

  std::cout << "VA-API version: " << major_version << "." << minor_version
            << std::endl;
  ...
  if (va_status != VA_STATUS_SUCCESS) {
    std::cerr << "Failed to create VA config: " << vaErrorStr(va_status)
              << std::endl;
    close(drm_fd);
    return false;
  }
  ...
  if (va_status != VA_STATUS_SUCCESS) {
    std::cerr << "Failed to create VA context: " << vaErrorStr(va_status)
              << std::endl;
    close(drm_fd);
    return false;
  }
...
bool WaylandHandler::SetupVulkan() {
  ...
  for (uint32_t i = 0; i < device_count; i++) {
    ...
    if (strstr(props.deviceName, "Intel") != nullptr) {
      ...
      std::cout << "Using Intel Vulkan device: " << props.deviceName
                << std::endl;
      break;
    }
  }
  ...
  if (vkCreateDevice(vulkan_physical_device_, &device_info, nullptr,
...
void WaylandHandler::RegistryGlobalHandler(void *data, wl_registry *registry,
  ...
  if (strcmp(interface, wl_compositor_interface.name) == 0) {
    handler->compositor_ = static_cast<wl_compositor *>(
        wl_registry_bind(registry, id, &wl_compositor_interface, 1));
  } else if (strcmp(interface, wl_shell_interface.name) == 0) {
    handler->shell_ = static_cast<wl_shell *>(
        wl_registry_bind(registry, id, &wl_shell_interface, 1));
  }
}

void WaylandHandler::RegistryGlobalRemoveHandler(void *data,
